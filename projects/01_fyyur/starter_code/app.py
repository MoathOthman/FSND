...
def venues():
  ...
  groups = []
  for k, g in groupby(venues, keyfunc):
  ...
  for g in groups:
    object = {
      'city': g[0].city,
      'state': g[0].state,
      'venues': map(withnum,g)
    }
    mapped.append(object)

  # print(f"mapped venues {list(mappedVenues)}")

  return render_template('pages/venues.html', areas=mapped);
...
def search_artists():
  ...
  searchterm = request.form.get('search_term', '')
  venues = Artist.query.filter(Venue.name.like(searchterm))
  def withnum(venue):
    showsCount = Show.query.filter(
      and_(
    ...
    return {'id': venue.id, "name": venue.name, "num_upcoming_shows": showsCount}
  venues = Artist.query.filter(Artist.name.ilike(f"%{searchterm}%")).all()
  mappedVenues = map(withnum,venues)
...
def shows():
  ...
  mapped = map(withVenueAndArtist, shows)
  return render_template('pages/shows.html', shows=mapped)
